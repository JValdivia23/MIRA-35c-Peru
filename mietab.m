...
bri_temp = linspace(-20,35,56); bri_nt=length(bri_temp); 
bri_n_n = interp_jairo(bri_temp,brit_temp,brit_n_n);
bri_n_k = interp_jairo(bri_temp,brit_temp,brit_n_k);
bri_nD = 800; bri_d = linspace(0.00001,0.008,bri_nD); %drop diamter in m
bri_kwq = interp_jairo(bri_temp,brit_temp,brit_kwq);


mietable = NaN(bri_nD,bri_nt);
sbcross = NaN(bri_nD,bri_nt);
raycross = NaN(bri_nD,bri_nt);
mietab = struct('mietable',mietable,'Drop_diameter',bri_d*1000,'Temperture',bri_temp,...
...
for it = 1:bri_nt
    ...
    for id = 1:bri_nD
        ...
        mietab.SBcross(id,it) = qback*pi^2*(bri_d(id))^2; %pi/4*D;
        mietab.RayCross(id,it) = bri_kwq(it)*bri_d(id)^6*pi^5/lambda^4;
    end
end
end
